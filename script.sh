...
sudo apt install certbot python3-certbot-nginx 2>/dev/null
echo $green_color"[SUCCESS]";
echo $green_color"[######################################]";
...
sudo ufw status >> $script_log_file 2>/dev/null
sudo ufw allow 'Nginx Full' >> $script_log_file 2>/dev/null
sudo ufw delete allow 'Nginx HTTP' >> $script_log_file 2>/dev/null
sudo ufw status >> $script_log_file 2>/dev/null
